...
#| tbl-cap: 'List of 52 most highly ranked law journals according to Gröls/Gröls (2009)'
...
As our journal benchmark set does not help in these respect, a further - item based - benchmark set is needed. For this, we contacted a number of publishers of journals in the journal benchmark set and asked for their original data. Three publishers provided data, for a total of seven journals:
...
References are the bibliographic data on the literature cited in a publication. Unfortunately, we observe that only 2.72% of all articles item in OpenAlex that belong to journals in our benchmark set contain data that indicates that they have (at least one) reference. As it is very unlikely that all other articles do not have references at all, the coverage of reference data seems to be extremely low.
...
!!! Der nächste Absatz muss nochmal besser erklärt werden - ich verstehe ihn nicht mehr !!!

We also had a look at the reference data with respect to the publication years to see if more recent publications do have a better coverage of references but this is not the case. @tbl-oa-references-statistics shows that 5,097 distinct articles appear as a reference in at least one article of our benchmark set, of which 41.55% are in the benchmark set; 43.24% of them are publications in journals we found by classification-based search (German, law) in OpenAlex. However, it must be kept in mind that these figures do not mean much because the reference data is simply to scarce for any kind of robust interpretation.
...
Looking at our results, it is clear that OpenAlex is the only of the bibliometric databases examined which currently has a minimal coverage of German law journals as far as our benchmark set is representative. It offers a limited, but steadily growing basis for bibliometric queries.
...
The authors would like to thank Ivo Vogel and Christian Mathieu, both Staatsbibliothek Berlin, for valuable feedback. They acknowledge the use of Large Language Models for English language editing and correction, for conceptual refinement, and for generating part of the code, in particular for the visualizations.