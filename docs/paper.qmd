---
title: Law Doesn't Count? Measuring Bibliometric Coverage of German Law Journals
author:
...
df.columns = ['Journal', 'Articles', 'German+Law', '%']
df['Journal'] = df['Journal'].astype(str).apply(lambda text: textwrap.shorten(text, width=50, placeholder="\u2026"))
df.style.hide(axis='index')\
  .set_properties(subset=df.columns[[0]], **{'text-align': 'left'})\
  .set_properties(subset=df.columns[[1,2]], **{'text-align': 'center'})\
  .set_properties(subset=df.columns[[3]], **{'text-align': 'right'})\
  .format({df.columns[3]: "{:.0%}"})
```
...
df.columns = ['Journal', 'Articles', 'German+Law', '%']
df['Journal'] = df['Journal'].astype(str).apply(lambda text: textwrap.shorten(text, width=90, placeholder="\u2026"))
df.style.hide(axis='index')\
  .set_properties(subset=df.columns[[0]], **{'text-align': 'left'})\
  .set_properties(subset=df.columns[[1,2]], **{'text-align': 'center'})\
  .set_properties(subset=df.columns[[3]], **{'text-align': 'right'})\
...
df = df.sort_values('law_and_de_article_share', ascending=False, ignore_index=True).head(20)
df.columns = ['Journal', 'Articles', 'German+Law', '%']
df['Journal'] = df['Journal'].astype(str).apply(lambda text: textwrap.shorten(text, width=50, placeholder="\u2026"))
df.style.hide(axis='index')\
  .set_properties(subset=df.columns[[0]], **{'text-align': 'left'})\
...
As this distinction is irrelevant to our purposes, we aggregated the different rankings into one list of 51 journals, thereby constituting our benchmark set (@tbl-benchmark-set).

```{python}
#| label: tbl-benchmark-set
#| tbl-cap: 'List of 51 most highly ranked law journals according to Gröls/Gröls (2009), sorted by abbreviation'

import pandas as pd
import textwrap
df = pd.read_csv("../data/benchmark/list-groels-groels-2009.csv")
df = df[['abbreviation', 'title']]
df.columns = ['Abbreviation', 'Title']
df = df.sort_values(by='Abbreviation').reset_index(drop=True)
df['Title'] = df['Title'].astype(str).apply(lambda text: textwrap.shorten(text, width=80, placeholder="\u2026"))
df.style.hide(axis='index')\
  .set_properties(subset=df.columns[[1]], **{'text-align': 'left'})
```

As stated earlier, one possible objection to using the study by Gröls & Gröls for our benchmark set is its age. Specifically, there could be the risk of missing relevant journals established after 2009 or not accounting for the fact that more recent rankings would vastly diverge from one fifteen years ago. In fact, for domain experts it seems an obvious problem that journals such as *Rechtswissenschaft* (est. 2010) or *Recht und Zugang* (est. 2020), are necessarily not included in our benchmark set and thus are not part of our study.
...
If we compare the benchmark set with the lists we created with a classification-based search in the databases Web of Science, Scopus and OpenAlex (@tbl-benchmark-set-coverage), we can make the following observations:
...
```{python}
#| label: tbl-benchmark-set-coverage
#| tbl-cap: 'Coverage of benchmark set journals in the bibliometric databases'

import pandas as pd
gs_size = pd.read_csv("../data/benchmark/list-groels-groels-2009.csv").shape[0]
data = []
...
colors = np.where(df.values=='✓', 'green', np.where(df.values=='✗', 'white', 'white'))
...
For completeness, we also checked how well the benchmark set aligned with the online lists: Matching again on the journal abbreviations (with all the associated problems) we can see the overlaps of the benchmark set with those sources in @fig-inclusion-benchmark-set-online-lists:

```{python}
#| label: fig-inclusion-benchmark-set-online-lists
...
data = {
    'groels+groels (2009)': '../data/benchmark/list-groels-groels-2009.csv', 
    'jura-recherche.de': '../data/web_lists/jura-recherche.de-2024-12-13.csv',
    'juris (2002)': '../data/web_lists/juris-ingested-journals-2002.csv',
    'openjur.de': '../data/web_lists/openjur.de-2024-12-13.csv',
    'wikipedia': '../data/wikipedia/category-law-journal-germany-with-issn.csv'
...
# update 13.11.2025: fixed wrong plots of NJW, JuS, and others: 
#  - missing publication data in journal_publication_periods.csv
#  - wrong calculation of max publication year
...
def compare_coverage_kb_zdb(df_kb, df_zdb, min_year, max_year):
    ...
    y_vals = df_kb['abk']
    for abk in y_vals:
        # Match abbreviations in lowercase, handling potential None values
        if df_zdb[df_zdb['abbr'].str.lower() == abk.lower()].empty:
            # Getting the name of the abbreviation
            name = df_kb[df_kb['abk'] == abk]['zeitschrift'].values[0]

            # Checking if the name exists in the 'name' column of df_zdb, comparing in lowercase
            if not df_zdb[df_zdb['name'].str.lower() == name.lower()].empty:
                # If so, we write the abbreviation into the 'abbr' column
                df_zdb.loc[df_zdb['name'].str.lower() == name.lower(), 'abbr'] = abk
    ...
    for y in y_vals:
        ...
        # Get min/max years from ZDB for fallback
        # Initialize with a high value to find the true minimum
        min_zdb_year = float('inf')
        max_zdb_year = df_temp['year_end'].max() if not df_temp.empty else None
        ...
        for _, row in df_temp.iterrows():
            ...
            # Store the overall min/max for this journal for OpenAlex fallback
            min_zdb_year = min(min_zdb_year, x1)
            max_zdb_year = max(max_zdb_year, x2) if max_zdb_year is not None else x2
            ...
            ax.plot(
              ...
              label="Publication period (source: ZDB)"
            )

        if min_zdb_year != float('inf'):
            zdb_min_years[y] = min_zdb_year

    # Plot available years from OpenAlex
    for index, row in df_kb.iterrows():
        y = row['abk']
        in_openalex = row['in_openalex']
        x1_oa = row['min_pubyear_in_openalex']
        x2_oa = row['max_pubyear_in_openalex']

        if in_openalex:
            df_zdb_journal = df_zdb[df_zdb['abbr'].str.lower() == y.lower()]
            # Fallback to ZDB years if OpenAlex years are missing
            x1 = x1_oa if pd.notna(x1_oa) else (df_zdb_journal['year_start'].min() if not df_zdb_journal.empty else None)
            
            # Handle fallback for end year, defaulting to max_year if NaN
            zdb_max_year = df_zdb_journal['year_end'].max() if not df_zdb_journal.empty else None
            x2 = x2_oa if pd.notna(x2_oa) else (zdb_max_year if pd.notna(zdb_max_year) else max_year)


            # Ensure OpenAlex start year does not precede ZDB start year
            if y in zdb_min_years and pd.notna(x1):
                x1 = max(x1, zdb_min_years[y])

            if pd.notna(x1) and pd.notna(x2):
                ax.plot((int(x1), int(x2)), (y, y), color='g', linewidth=5, label="Available metadata in OpenAlex")
    ...
    # Set y-axis limits to remove top/bottom padding
    ...
    plt.legend(by_label.values(), by_label.keys(), loc='upper center', bbox_to_anchor=(0.5, 1.05), ncol=2)
...
data.append(['Percentage of OpenAlex article items in benchmark set with at least one reference', percentage_items_with_refs])
...
data.append(['Cited publications in the classification-based search', sqldf("""
  select round(1000*
  (select count(distinct item_id_cited) 
  from eva_refs_in_openalex
...
df = pd.DataFrame(data, columns=['Query', 'Result'])
df.style.hide(axis='index')\
  .set_properties(subset=df.columns[[0]], **{'text-align': 'left'})\
  .set_properties(subset=df.columns[[1]], **{'text-align': 'right'})\
  .format({df.columns[1]: "{:.6}"})
```
...
data.append(['Share of benchmark set articles cited in OpenAlex', sqldf("""
  select round(10000*
...
data.append(['Share of citing OpenAlex articles in benchmark set', sqldf("""
  select round(10000*
...
data.append(['Share of citing OpenAlex articles in the classification-based search', sqldf("""
  select round(10000*
  (select count(distinct item_id_citing) 
  from eva_citations_in_openalex
...
@fig-openalex-citations displays the absolute differences in citation counts between the journals, without accounting for the varying total number of articles per journal, which would be necessary for generating comparable metrics.

```{python}
#| label: fig-openalex-citations
#| fig-cap: 'Share of articles with at least one citation per journal in the benchmark set'
...
#| fig-cap: 'Word cloud of OpenAlex keywords in the benchmark set articles'
...
We compared the Gröls/Gröls ranking with the JIF-based ranking from OpenAlex separately for the generalist journals (@fig-comp-groels-jif-general) and the specialized ones (@fig-comp-groels-jif-specialized). Looking at the results, the rankings differ considerably.
...
# general journals
jura_groels_tabelle = pd.read_csv(f'../data/benchmark/jura_groels_tabelle1_allg_zeitschriften.csv', low_memory=False)
...
jura_groels_tabelle = pd.read_csv(f'../data/benchmark/jura_groels_tabelle2_fachspez_zeitschriften.csv', low_memory=False)
compare_rankings(sqldf(query), label_ranking_a='Ranking JIF OpenAlex', label_ranking_b='Ranking Gröls/Gröls')
...
[^28]: Whether German legal scholarship really can do without Peer Review because in "Jurisprudence knows no right or wrong" [@gulden2023] is another question.
